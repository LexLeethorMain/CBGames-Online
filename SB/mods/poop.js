    ...
    tool: function(pixel) {
        ...
        for (var i = 0; i < adjacentCoords.length; i++) {
            ...
            if (!isEmpty(x, y)) {
                ...
                var temp = pixel.element;
                pixel.element = adjacentPixel.element;
                adjacentPixel.element = temp;
            }
        }
    }
};
...
if (!settings.survival) {
 ...
 if (settings.survivalClone) {
        if (settings.survivalClone === "diamond") {
            settings.survivalClone = null;
            alert('Why are you cloning diamonds?');
        }
...
elements.cloner.ignore = elements.cloner.ignore.concat(["gold","gold_coin","molten_gold","sun","supernova","diamond", "cloner"]);
